...
const ExecutionPrice = Type.Optional(
  Type.Number({
...
const Notes = Type.Optional(
  Type.String({ description: "Optional free-form notes." }),
);

/** Idempotency key required for all side-effecting requests. */
const IdempotencyKey = Type.String({
  description:
    "Unique idempotency key (UUID recommended). Duplicate keys safely replay the same side effect.",
  minLength: 1,
  maxLength: 128,
});
...
export const ListPositionsParams = Type.Object(
  {},
  { description: "List all current projected positions." },
);
...
export const WeeklyReviewParams = Type.Object(
  {},
  { description: "Fetch the latest weekly review with open positions." },
);
