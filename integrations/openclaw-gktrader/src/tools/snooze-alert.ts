...
export const snoozeAlertTool: ToolPluginToolDefinition<
  { apiBaseUrl: string; sharedSecret: string },
  typeof SnoozeAlertParams
> = {
  name: "gktrader_snooze_alert",
  label: "Snooze Alert",
  description:
    "Snooze a GKTrader alert for a configurable number of minutes (1-1440, default 30). Requires an idempotency key.",
  ...
  async execute(params, config, _context) {
    ...
    const result = await client.snoozeAlert(
      params.alert_id,
      params.minutes,
