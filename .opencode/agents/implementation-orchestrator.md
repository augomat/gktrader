...
4. Launch `implementation-worker` tasks in parallel when scopes are independent. Each worker prompt should include:
   - the exact objective
...
7. After worker tasks finish, integrate any missing glue yourself and resolve conflicts before validation.
...
10. If anything remains off-spec or under-verified, fix it yourself or delegate the repair work back to `implementation-validator`, then validate again.
